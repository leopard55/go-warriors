...
public class SuperArray<T> {
    ...
    private T[] array; //自定义数组底层构建数组。= new String[DEFAULT_CAPACITY]，先不初始化，在构造的时候再初始化

    private int size;

    public SuperArray() {  //构造方法的是目的是创建对象
        super();
        capacity = DEFAULT_CAPACITY;
        array = (T[]) new Object[DEFAULT_CAPACITY]; //无参默认创建的数组长度为10
    }

    public SuperArray(int capacity) {
        super();
        this.capacity = capacity;
        array = (T[]) new Object[capacity]; // Object表示广泛，泛型 类型不存在，只在编译期有效，运行期无效
    }
    ...
    public T remove(int index) {
        rangeCheck(index);
        T removeObj = array[index]; //即将要删除的元素
